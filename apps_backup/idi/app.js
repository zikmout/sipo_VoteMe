...
var contractsManager = erisC.newContractManagerDev(erisdbURL, accountData.tot_root_000);

// properly instantiate the contract objects using the abi and address
var idisContract = contractsManager.newContractFactory(idisAbi).at(idisContractAddress);
/*
idisContract.time(function (err, time_it) {
	if (err) throw err;
	else console.log(time_it);
});*/

function getVoters() {
	idisContract.getTotalVoters(function(err, msg){
		if (err) throw err;
		else console.log("TOTAL VOTERS: ", parseInt(msg, 16));
	});
}

idisContract.getEndDate(function(err, msg){
	if (err) throw err;
	else {
...
idisContract.getStartDate(function (err, msg) {
	if (err) throw err;
	else {
...
idisContract.getQuestion(function(err, msg) {
	if (err) throw err;
	console.log("Question Of the POLL is: ");
	console.log(new Buffer(msg, 'hex').toString('utf8'));
});
